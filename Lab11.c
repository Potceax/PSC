...
void FillArrayWithRandom(int *tab, int start, int end)
{
	srand(time(NULL));
...
bool isSumOfCubeElementsEqualNumber(int number, int *tab, int length)
{
	int sum = 0;
...
#define M 100
	...
	StartInfo("Program which prints a sum and multiplication of an array with n length");
...
#define O 3			// number of all three digit numbers
	...
	StartInfo("Program which prints all numbers which are the sum of cubes of their numbers\n");
	...
	for (int n = 100; n <= 999; ++n) {
		...
		if(isSumOfCubeElementsEqualNumber(n, tab, O))
			printf("%d ", n);
	}
	printf("\n\n");
	EndInfo();
